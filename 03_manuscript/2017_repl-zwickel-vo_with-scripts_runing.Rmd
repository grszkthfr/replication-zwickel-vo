...
present study, we tried to modulate viewing behavior via task instructions **[for a similar procedure see @Flechsenhar2017]**.
...
resulting in **less** prioritization of the **cued** object. Furthermore, we
...
object **was** expected to be better compared to memory performance of the uncued
...
examining 90 participants (plus eventual dropouts) to achieve a power of .80
...
between $`r min_age`$ and $`r max_age`$ years ($M = `r m_age`$ years, $SD = `r
sd_age`$ years). Overall, participants scored very low for **autism** traits in
the Autism-Spectrum Quotient scale [AQ-k, German version, @Freitag2007, Range 0
to 23, $M = `r m_aqk`$, $SD = `r sd_aqk`$]. **In the final sample, one
...
across all scenes (see online supplement for a complete list of all objects).
The direction of the gaze (left/right) and the placement of the objects (object
A and B left/right) were balanced by taking 4 photographs of each scene (see
Figure\ \@ref(fig:FigExampleStimulus) for an example). **Similar to
@Zwickel2010, we did not restrict the position of the individual in the
...
Eye movements were tracked with the corneal reflection method and were recorded
with an EyeLink1000plus tower system at, a sampling rate of 1000 Hz. The
stimulation was controlled via Presentation® (Neurobehavioral Systems). All
stimuli had a resolution of 1280 x 960 pixels and were displayed on a 24" LG
...
individual and landed on the cued and uncued objects, respectively.
...
duration (i.e., range `r round(min(df_mem_glmm$prop_dur_z),2)` to
`r round(max(df_mem_glmm$prop_dur_z),2)`) in model 2a and analogously, the
...
object role and corresponding interaction terms with the other factors to the
previous models. We always tested the higher-order model against its lower-order
counterpart using an ANOVA approach to examine if relative fixation duration
and/or relative number of fixations had incremental value beyond group
...
effect sizes generalized eta-square ($\eta^2_G$) for ANOVAs are reported, where
guidelines suggest .26 as a large, .13 as a medium and .02 as a small effect
[@Bakeman2005].
...
for model parameters and model selecetion criteria). Next, we built two extended
...
significance. By contrast, the main effect of number of fixations improved the
prediction of recalled stimuli. Again, the interaction of group and number of
fixations did reach significance. For number of fixations we tested as a last
step, whether object role further improves the prediction of recall performance,
which was not the case. Neither the main effect of object role, nor its
interactions with the other factors reached statistical significance.**
...
# latency
apa_aov_sa_lat <-
    apa_print(aov_sa_lat, correction="GG", mse = FALSE)$full_result
apa_emm_sa_lat <-
    apa_print(emm_sa_lat$contrasts)$statistic

# duration
apa_aov_sa_dur <-
    apa_print(aov_sa_dur, correction="GG", mse = FALSE)$full_result
apa_emm_sa_dur <-
    apa_print(emm_sa_dur$contrasts)$statistic

# number
apa_aov_sa_num <-
    apa_print(aov_sa_num, correction="GG", mse = FALSE)$full_result
apa_emm_sa_num <-
    apa_print(emm_sa_num$contrasts)$statistic
...
`r m_sa_lat_head` ms$, $M_{lat,body} = `r m_sa_lat_body` ms$)**. There was neither a
...
(`r apa_aov_sa_dur$group_id`), nor for the number of fixations,
...
**To assess temporal changes of instruction effects on social prioritization we
analyzed fixation duration and number across time points (see Figure\
\@ref(fig:FigFixTmp)). For both we observed a significant interaction between
group and time points (duration: `r apa_aov_sa_dur$group_id_bin_id`, number:
`r apa_aov_sa_num$group_id_bin_id `) as well as between ROI (head/body) and time
points (duration: `r apa_aov_sa_dur$fix_id_bin_id `, number: 
`r apa_aov_sa_num$fix_id_bin_id `) . Pairwise t-test revealed that the interaction
between group and time points was driven by the first and last time point (first
time point: `r apa_emm_sa_dur$free_mem_head_X1`, last time point:
`r apa_emm_sa_dur$free_mem_head_X5 `, with all other time points $p > .09$). For
fixation number on the head, surprisingly the first and second time point showed
significant differences across group assignment (first:
`r apa_emm_sa_num$free_mem_head_X1`, second: `r apa_emm_sa_num$free_mem_head_X2`,
except the last time point ( `r apa_emm_sa_num$free_mem_head_X5`), all other
time points $p > .14$). None of the contrasts for body reached significance
across instruction group and time points (all $p > .28$).**
...
between our manipulations were most prominent in the first 2 seconds (see
Figure\ \@ref(fig:FigFixTmp)). During that time, fixations on the head differed
clearly between instruction groups, with more social prioritization during free
viewing. Temporal analysis for gaze following showed in the first 2 seconds
strongest preference of the cued object in both groups. Interestingly, viewing
pattern differed between both groups after the initial 2 seconds. Object
prioritization was reversed during the second time point (seconds 2 - 4) in the
explicit encoding group with more and longer fixations on the uncued object. In
contrast, participants in the free viewing group payed as much attention on the
uncued object as on the cued object during that time period. It seems that
consistent with our hypothesis, the instruction induced a more systematic
exploration of the objects in the explicit encoding group resulting in a
reduction of gaze following.** All these findings together indicate a strong and
automatic bias of using gaze cues for attentional guidance **especially in the
early phases of stimulus exploration. From the temporal analysis of attentional
allocation, we can conclude, that the unexpected missing interaction between
group and object role in the analyses of the whole viewing period most likely
stems from the extended presentation duration. Our results indicate a rather
strong group effect in the first 4 seconds, especially regarding object role.
With respect to fixations on the head, however, the picture is less clear. The
instruction manipulation seemed to affect only very early periods of stimulus
processing with attentional effects not persisting over time.**
...
human being was not relevant to this task, we expected a generally reduced attention
...
prediction of stimulus recall over group assignment (see Table\
\@ref(tab:tblGLMM)). Fixation duration did not add incremental value**. This is
...
object to be better recalled than the uncued object [@Schmidt2002;
...
interpretation finds support in the fact that the group assignment primarily
...
the unbiased free viewing condition. These results indicate that the mere
presence of other human beings as well as their gaze orientation have a strong
