...
source(path(path_scripts_publ, "dscr_stim.R"))

m_stim_cued <- round(dscr_stim[dscr_stim$fix_id == "object_cued",]$m_size*100, 1) %>% 
    as.character()
sd_stim_cued <- round(dscr_stim[dscr_stim$fix_id == "object_cued", ]$sd_size*100, 1) %>% 
    as.character()
m_stim_uncued <- round(dscr_stim[dscr_stim$fix_id == "object_uncued", ]$m_size*100, 1) %>% 
    as.character()
sd_stim_uncued <- round(dscr_stim[dscr_stim$fix_id == "object_uncued", ]$sd_size*100, 1) %>% 
    as.character()
m_stim_head <- round(dscr_stim[dscr_stim$fix_id == "head", ]$m_size*100, 1) %>% 
    as.character()
sd_stim_head <- round(dscr_stim[dscr_stim$fix_id == "head", ]$sd_size*100, 1) %>% 
    as.character()
m_stim_body <- round(dscr_stim[dscr_stim$fix_id == "body", ]$m_size*100, 1) %>% 
    as.character()
sd_stim_body <- round(dscr_stim[dscr_stim$fix_id == "body", ]$sd_size*100, 1) %>% 
    as.character()
...
# object role
m_ja_lat_cued <-
    round(mean(dscr_ja_lat[dscr_ja_lat$fix_id == "object_cued",]$m_lat), 4) * 100
m_ja_lat_uncued <-
    round(mean(dscr_ja_lat[dscr_ja_lat$fix_id == "object_uncued",]$m_lat), 4) * 100

m_ja_dur_cued <-
    round(mean(dscr_ja_dur[dscr_ja_dur$fix_id == "object_cued",]$m_dur), 4) * 100
m_ja_dur_uncued <-
    round(mean(dscr_ja_dur[dscr_ja_dur$fix_id == "object_uncued",]$m_dur), 4) * 100

m_ja_num_cued <-
    round(mean(dscr_ja_num[dscr_ja_num$fix_id == "object_cued",]$m_num), 4) * 100
m_ja_num_uncued <-
    round(mean(dscr_ja_num[dscr_ja_num$fix_id == "object_uncued",]$m_num), 4) * 100

# group
m_ja_lat_free <-
    round(mean(dscr_ja_lat[dscr_ja_lat$group_id == "free",]$m_lat), 4) * 100
m_ja_lat_mem <-
    round(mean(dscr_ja_lat[dscr_ja_lat$group_id == "mem",]$m_lat), 4) * 100

m_ja_dur_free <-
    round(mean(dscr_ja_dur[dscr_ja_dur$group_id == "free",]$m_dur), 4) * 100
m_ja_dur_mem <-
    round(mean(dscr_ja_dur[dscr_ja_dur$group_id == "mem",]$m_dur), 4) * 100

m_ja_num_free <-
    round(mean(dscr_ja_num[dscr_ja_num$group_id == "free",]$m_num), 4) * 100
m_ja_num_mem <-
    round(mean(dscr_ja_num[dscr_ja_num$group_id == "mem",]$m_num), 4) * 100

# saccades
# object role
m_sac_cued <-
    round(mean(dscr_sac[dscr_sac$fix_id == "object_cued",]$m_sac), 4) * 100
m_sac_uncued <-
    round(mean(dscr_sac[dscr_sac$fix_id == "object_uncued",]$m_sac), 4) * 100

# group
m_sac_free <-
    round(mean(dscr_sac[dscr_sac$group_id == "free",]$m_sac), 4) * 100
m_sac_mem <-
    round(mean(dscr_sac[dscr_sac$group_id == "mem",]$m_sac), 4) * 100
...
`r apa_aov_ja_lat$fix_id`; $M_{lat,cued} = `r m_ja_dur_cued`$, $M_{lat,uncued} 
= `r m_ja_dur_uncued`$)** objects. The main effect of instruction group was also
significant, with earlier fixations on both objects
in the explicit encoding **(`r apa_aov_ja_lat$group_id`; $M_{lat,mem} = 
`r m_ja_dur_mem`$)** compared to the free viewing group **($M_{lat,free} = 
`r m_ja_dur_free`$)**. The interaction effect failed to reach statistical
...
# head vs body
m_sa_lat_head <-
    round(mean(dscr_sa_lat[dscr_sa_lat$fix_id == "head",]$m_lat))
m_sa_lat_body <-
    round(mean(dscr_sa_lat[dscr_sa_lat$fix_id == "body",]$m_lat))

m_sa_dur_head <-
    round(mean(dscr_sa_dur[dscr_sa_dur$fix_id == "head",]$m_dur), 4) * 100
m_sa_dur_body <-
    round(mean(dscr_sa_dur[dscr_sa_dur$fix_id == "body",]$m_dur), 4) * 100

m_sa_num_head <-
    round(mean(dscr_sa_num[dscr_sa_num$fix_id == "head",]$m_num), 4) * 100
m_sa_num_body <-
    round(mean(dscr_sa_num[dscr_sa_num$fix_id == "body",]$m_num), 4) * 100

# group
m_sa_lat_free <-
    round(mean(dscr_sa_lat[dscr_sa_lat$group_id == "free",]$m_lat), 4) * 100
m_sa_lat_mem <-
    round(mean(dscr_sa_lat[dscr_sa_lat$group_id == "mem",]$m_lat), 4) * 100

m_sa_dur_free <-
    round(mean(dscr_sa_dur[dscr_sa_dur$group_id == "free",]$m_dur), 4) * 100
m_sa_dur_mem <-
    round(mean(dscr_sa_dur[dscr_sa_dur$group_id == "mem",]$m_dur), 4) * 100

m_sa_num_free <-
    round(mean(dscr_sa_num[dscr_sa_num$group_id == "free",]$m_num), 4) * 100
m_sa_num_mem <-
    round(mean(dscr_sa_num[dscr_sa_num$group_id == "mem",]$m_num), 4) * 100
