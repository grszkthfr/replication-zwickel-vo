...
abstract: |
  ...
  the scene received a large amount of attention, especially during early phases
  of visual attention, even though they were irrelevant to the current task.
  These results indicate that the mere presence of other human beings as well as
  their gaze orientation have a strong impact on attentional exploration.
  Data and analysis scripts are available at [https://osf.io/jk9s4](https://www.osf.io/jk9s4)
...
resulting in higher prioritization of both objects and reduced cueing effects.
...
had normal or corrected-to-normal vision and were recruited via the University of
...
across all scenes (see [online supplement](https://osf.io/b73ea/) S1 for a
complete list of all objects). The direction of the gaze (left/right) and the
...
outdoor example). Similar to @Zwickel2010, we did not restrict the position of
the individual in the photograph (i.e., the person could appear in the center or
more peripherally) such that participants could not expect a specific spatial
...
corneal reflection method and were recorded with an EyeLink 1000plus system
(tower mount) at a sampling rate of 1000 Hz. The stimulation was controlled via
Presentation® (Neurobehavioral Systems). All stimuli had a resolution of 1280 x
960 pixels and were displayed on a 24" LG 24MB65PY-B screen (resolution: 1920 x
1200 pixels, display size: 516.9 x 323.1 mm) with a refresh rate of 60 Hz. The
viewing distance amounted to 50 cm thus resulting in a visual angle of 38.03° x
28.99° for the photographs.
...
[https://osf.io/jk9s4/](https://osf.io/jk9s4/). For the analysis of the
eye-tracking data, EyeLink's standard configuration was used to parse eye
movements into saccades and fixations. Saccades were defined as eye movements
exceeding a velocity threshold of 30 °/s or an acceleration threshold of 8.000
°/s². Fixations were defined as time periods between saccades.
...
analyses of variance (ANOVAs) including the between-subject factor instruction
...
ANOVAs were computed with the package *afex* [@R-afex]. As effect sizes,
generalized eta-square ($\hat{\eta}^2_G$) values are reported, where guidelines
suggest .26 as a large, .13 as a medium and .02 as a small effect
[@Bakeman2005]. Greenhouse-Geisser correction was applied in all
...
`r apa_emm_ja_dur_t_fix_id_bin_id$X1_object_cued_object_uncued`,
number:
`r apa_emm_ja_num_t_fix_id_bin_id$X1_object_cued_object_uncued`,
last time point: duration:
`r apa_emm_ja_dur_t_fix_id_bin_id$X5_object_cued_object_uncued`,
number:
`r apa_emm_ja_num_t_fix_id_bin_id$X5_object_cued_object_uncued`,
...
`r apa_emm_ja_dur_t_group_id_bin_id$X1_free_mem`,
numbers:
`r apa_emm_ja_num_t_group_id_bin_id$X1_free_mem`,
third: duration:
`r apa_emm_ja_dur_t_group_id_bin_id$X3_free_mem`,
numbers:
`r apa_emm_ja_num_t_group_id_bin_id$X3_free_mem`,
fourth: duration:
`r apa_emm_ja_dur_t_group_id_bin_id$X4_free_mem`,
numbers:
`r apa_emm_ja_num_t_group_id_bin_id$X4_free_mem`,
and last time point: duration:
`r apa_emm_ja_dur_t_group_id_bin_id$X5_free_mem`,
numbers:
`r apa_emm_ja_num_t_group_id_bin_id$X5_free_mem`).
...
fixation duration and its interaction with group did not yield a better
prediction of recall performance in comparison to model 1
(`r apa_aov_mem_1.2`).
...
(`r apa_emm_sa_num_1_group_id_fix_id$free_mem_head`)
...
[online supplement](https://osf.io/b73ea/), Tables S8 - S12).
...
attention still occurred, and were even largely comparable to the unbiased free
viewing condition. These results indicate that the mere presence of other human
