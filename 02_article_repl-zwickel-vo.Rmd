...
```{r Anova fixations, warning=FALSE, message=FALSE}
...
```
The relative cumulative fixation duration is calculated as time duration on an OOI divided by the time spent fixating the rest of the scene. It shows a bias towards the gazed-at object, which was fixated significantly longer (see Table/Figure). The corresponding ANOVA revealed a significant main effect for reference, `r apa.fix.gaze$full_result$icond` and for condition `r apa.fix.gaze$full_result$df_w_et_group` which were unaffected by the two-way interaction, `r apa.fix.gaze$full_result$df_w_et_group_icond`.

### Fixation latency
An additional measurement of priorisation is fixation latency, that is for each OOI  the mean out of all first fixations after stimulus onset. It showed, an object is fixated earlier when it is gazed-at or when the observer was in the explicit encoding condition. Both is confirmed by an ANOVA: for reference `r apa.fixlat.gaze$full_result$icond` and condition, `r apa.fixlat.gaze$full_result$df_w_et_group`, but again there was no interaction, `r apa.fixlat.gaze$full_result$df_w_et_group_icond`.

### Fixation number
As a third measurement of priorisation, fixation number is the count of fixations per ROI during scene presentation, divided by the total number of fixations during scene presentation. They show, that OOI were fixated more often when gazed-at `r apa.fixn.gaze$full_result$icond`, and that objects were more often fixated in the explicit encoding condition`r apa.fixn.gaze$full_result$df_w_mem`. And again there is no interaction, `r apa.fixn.gaze$full_result$df_w_et_group_icond`
...
Observers reminded OOI equally well, regardless of reference `r apa.mem$full_result$df_w_mem_bed`or instrucition `r apa.mem$full_result$imem`.


## Head and body
A similar pattern to the OOI can be seen when comparing the head with the body region. Again there were the same measurements to acount for priorisation, but in this case only t-test were calculated, because there were no specific hypotheses to account for the instructions.
