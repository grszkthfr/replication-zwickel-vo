...
```
The free-recall test showed, that observers with explicit encoding remembered on average 2 items more then observers from the other group. In an Anova this effect was significant, `r apa.mem$full_result$df_w_mem_bed`. No effect is found for referencing an OOI, it did not influence memory performance `r apa.mem$full_result$imem`. There was also no interaction, `r apa.mem$full_result$df_w_mem_bed_imem`.
