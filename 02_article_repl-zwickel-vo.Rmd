...
for (st in seq(5,16,4)) { # Variables 5:16, every 4th: fix.face and fix.noface; fixn.face and ...
  carmod <- lm(as.matrix(df.w.et[,st:(st+1)]) ~ df.w.et$group)
  #print(Anova(carmod, idata=idata, idesign=~icond, type="III"))
  anova.fixface <- append(anova.fixface, list(carmod))
  }

print("fix.face")
print(Anova(anova.fixface[[1]], idata=idata, idesign=~icond, type="III"))
apa.fix.face <- apa_print(Anova(anova.fixface[[1]], idata=idata, idesign=~icond, type="III"))

print("fixn.face")
print(Anova(anova.fixface[[2]], idata=idata, idesign=~icond, type="III"))
apa.fixn.face <- apa_print(Anova(anova.fixface[[2]], idata=idata, idesign=~icond, type="III"))


print("fixlat.face")
print(Anova(anova.fixface[[3]], idata=idata, idesign=~icond, type="III"))
apa.fixlat.face <- apa_print(Anova(anova.fixface[[2]], idata=idata, idesign=~icond, type="III"))

df.w.et %>% summarise(mean(fix.face), mean(fix.body), mean(fixn.face), mean(fixn.body), mean(fixlat.face), mean(fixlat.body))
df.w.et %>% group_by(group) %>% summarise(mean(fix.face), mean(fix.body), mean(fixn.face), mean(fixn.body), mean(fixlat.face), mean(fixlat.body))
