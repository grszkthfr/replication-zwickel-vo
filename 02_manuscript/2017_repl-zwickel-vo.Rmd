...
<!-- erweiterung topdown -->
However, a direct replication was not the only motivation for this experiment. Additionally, it aims to test the pattern in gaze behavior as it was shown here and in @Zwickel2010 against top-down modulation. That the top-down modulation was applied successful, can not only be seen in the better performance recalling objects of observers in that group. Also the viewing behavior changed, from a to a more systematic pattern in the explicit encoding group. Specifically, objects gain more attention in contrast to the free-viewing condition, with out any explicit modulation. Additionally, the head losses some of its natural salience, consistent for all measures. Interestingly, however, top-down-modulation does not interact with either social attention nor joint attention measures consistent for all measures. The prioritization shown for the head and also for the gazed-at object remain stable, unaffected of this specific top-down modulation. So, the effects by social influence is even when observers investigate the scene systematic relevant. This evidence provides support how automatic and reflexive and can't be easily overwritten. The extension of this line of research, namely whether the gaze behavior can be influenced by top-down modulation. Consequently, this research is extended by the given study by an explicit task concerning the given scene. this modulation has an impact on gaze behavior.<!-- Einordnen mit privious findings && unclear was bei anderen top down modulationen! -->
