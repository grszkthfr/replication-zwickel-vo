...
abstract: |
  Humans as social beings rely on information provided by conspecifics. One important signal in social communication is eye gaze. The current study (n=90) sought to replicate and extend previous findings of attentional guidance by eye gaze in complex everyday scenes. In line with previous studies, we observed longer, more and earlier fixations for objects referenced by gaze in free viewing conditions. To investigate how robust this prioritization is against top-down modulation, half of the observers receive a memory task that required to scan the whole scene instead of exclusively focusing on gazed-at objects. Interestingly, similar cueing effects occurred in this condition. Moreover, the human beings depicted in the scene received a large amount of attention even though they were irrelevant to the current task. These results indicate that the mere presence of other human beings as well as their gaze orientation have a strong impact on attentional exploration.
...
In fact, this line of research is being extended. First, the influence of top-down modulations on joint and social attention in naturalistic scenes is investigated. Earlier research showed, that social attention is influenced by multiple factors. For example, social status [@Foulsham2010] or expectations [@Perez-Osorio2015] change the viewing behavior of the observer. These studies have in common that they indirectly manipulate the observers viewing behavior by manipulating the stimuli in one or the other way. For example @Foulsham2010 build the stimulus set from stimuli which were rated for social status.
In the present study, however, half of the observers recieve a very explicit instruction. This half of the observers (the *explicit-encoding group*) received the information that a memory test is following previous to the experimental part. The instruction made objects relevant for suceeding in the memory task. Attention towards the person would be inefficient. The manipulation is thought to induce a more explicit and systematic encoding of the presented scenes. Additionally, the consequences for latter cognitive processes, here memory effects, are examined for the different roles of the objects. The other half of the observers who did not receive any top-down modulation should provide an unaffected viewing behavior, just like @Zwickel2010. They did the memory task unannounced after the experimental part. For both groups the memory task was a free-recall test where they had to recall as many as things as possible from the scenes.
As @Zwickel2010 showed, a person in a scene influences the viewing behavior of observers spontaneously and without further instruction or manipulation. Here the motivation for the given instruction to complete a memory test was twofold. First, it is thought to influence the viewing behavior, in that sense, that spontaneous viewing behavior will be reduced and that it will be more systematic. It is important to note, that it is not expected that the influence of the presence of the person would vanish due to top-down modulation, but it is supposed to be weaker. Effects should be visible regardless for the social attention part and also for the joint attention part of this study.  Additionally, memory effects sensitive for the role of the object were examined, to answer the question, whether more attentional ressources on a object pays off with enhanced memorybility. This is mainly interesting for the free-viewing group with the unbiased viewing behaviour.
    <!-- Hypothesis -->
...
*H5*. Due to the instruction of the memory test, and the assumed change in processing the scene, the viewing behavior should change for social stimuli. Due to explicit encoding, it is expected that natural viewing behavior which is known to prioritize the head is reduced and therefore the head looses some of its salience. As a consequence, the head should be prioritized stronger in the free-viewing group compared to the explicit-encoding group. Resulting in longer fixations, faster fixations and more fixations. 
...
<!-- Replikation -->
As in @Zwickel2010 and others studies (for example @Birmingham2008), a strong prioritization can be seen for heads in the naturalistic scenes. Accordingly the head was fixated first, before any other ROI. Besides that it was also fixated more often, meaning that during scene presentation observers kept looking at the head. Consequently, no ROI was fixated longer in total then the head.
Additionally, also in line with @Zwickel2010 gazed-at objects were preferenced over the object not being gazed-at by the person in a given trial. The preference for the gazed-at object can be seen in several measures. First, the object which was gazed-at were fixated remarkably earlier. Furthermore, objects were also fixated longer with more frequent fixations when they were gazed-at. Therefore, the gaze cue by the person does not only leads to more thorough processing during the whole time of scene presentation, but also guides attentional resources very early. All these findings together indicate a strong prioritization through the gaze cue of the person in the scene.
Moreover, the compareable prioritization of the head and the gazed-at region already suggest a link between these two ROIs. To investigate a direct relation between these regions, the leaving saccades from the head are examined. Saccades leaving the head are more likely to end on the gazed-at object. This underlines a direct link between the head and the gazed-at object, and is again in line with @Zwickel2010. 
So far the given results replicate all aspects @Zwickel2010 demonstrated for persons in their study. However, this replication had considerably more power then the original study, used photographic scenes and included a top-down modulation. As a consequence smaller standard error and therefore more precise estimation of the effect sizes were provided here.
...
<!-- top-down -->
However, a conceptual replication was not the only motivation for this experiment. It aims at extending the line of research, generalising it to new stimuli and at testing the pattern in gaze behavior as it was shown here and in @Zwickel2010 against top-down modulation, specifically an instructed task prior free-viewing. Unsurprisingly the task scores show, that oberserves, who knew about the free-recall task in advance perform better in recalling items. Furthermore, viewing behavior was different depending on group membership. Specifically, obervers pay more attention to objects when they recieved the instruction prior to the viewing part, at the same time the head loses some of its natural salience. Both findings are consistent over all measures. These findings suggest, that observers with instruction had a more systematic gaze pattern, where they preferred ROIs relevant to the task, namely the objects. However, no interaction between top-down modualtion and ROI can be found. Social attention, as well as joint attention remain consistent for all measures stable. The prioritization shown for the head and also for the gazed-at object was unaffected by the given top-down modulation, although discreptive differences became smaller (note, that this trend was not significant). The attentional guidance of social and joint attentional prozesses can be seen, even when observers investigate the scene more systematic. This evidence provides support for the automaticity and reflexivness of the observed effects, even when the observer voluntarily aims at scanning the scene systematically. Nevertheless, observer with instructed free viewing performed better.However the contribuion of the attentional reallocation remains unclear. In particular, whether an object was cued or not by the person, did not influence its probability being recalled, note that cued objects recieved more attention. That is partially inconsistent with the hypothesis, which stated correct that porocessing changes, but missing the link that the change in processing for solving the task comes from the attentional reallocation of attention. Even as it is assumed, this findings do not indicate a link between spent attentional ressources and performance for recall. This missing link can mean WHAT!? <!-- korrelation zeit angeschaut und erinnert? --> <!-- Einordnen mit privious findings && unclear was bei anderen top down modulationen! -->
