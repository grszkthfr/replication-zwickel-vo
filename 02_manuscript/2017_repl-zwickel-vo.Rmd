...
#### Fixation duration
<!-- (H1, H2) -->
To qunatify and compare priorisations against each object a measuer on how big the proportion of the duration of all fixations was for every ROI was calculated. Specifically, the time duration on an object was divided by <!-- The relative cumulative fixation duration is calculated as time duration on an object divided by--> the time spent fixating the rest of the scene to get a  was for  fixated relative to all fixations.<!-- what do you get then!? -->  It shows a bias towards the gazed-at object, which was fixated longer (see Table/Figure) and confirms [@Zwickel2010]'s findings. As predicted the corresponding ANOVA revealed a significant main effect for reference, `r apa.anova.fix.gaze$full_result$icond` and for condition `r apa.anova.fix.gaze$full_result$df_w_et_group`. Against prediction their was no effect of the two-way interaction, `r apa.anova.fix.gaze$full_result$df_w_et_group_icond`. <!-- was it against pred? --> 
...
The main attempt of this study was to replicate effects from the mere presence of a person in a scene on gaze behaviour, what was first shown by @Zwickel2010. The findings in this study match findings from Zwickel. A strong preference for the gazed-at object can be seen in several measueres. First, a lower latency until first fixation can be seen for the gazed-at object, meaning that there is a urgent time sensitive priorisation. Furthermore, the gazed-at objects were also longer fixated and also more frequent fixated. therefore, these objects were also processed more thorughly during the whole time of scene presentation. These results replicate, what @Zwickel2010 already demonstrated in their study for joint attention. Overall, this finding puts more evidence on the idea that the viewing behavior is heavily but spontaneously influenced by persons in a scene.
For genuine social stimuli like the person, and more specifically the head and the body, a similar pattern can be observed. The head had the lowest latency until first fixation, and logically even shorter then the gazed-at object. This makes sense, since it is assumed, that the head leads the gaze of the observer to the gazed-at object. Additionally, the head was also fixated longer and more often then the body.
Saccades leaving the head were more likly to end on the gaze-at object, compared to the not-gazed-at-object.
However, a direct replication was not the only motivation for this experiment. Additionally, it is shown that the pattern in gaze behavior Zwickel et al show, can be generalised to photographic scenes. The extension of this line of research, namly whether the gaze behavior can be influenced by top-down modulation. Consequently, this research is extended by the given study by an explicit task concerning the given scene. this modulation has an impact on gaze behavior. The objects receive more attention overall. The gaze behavior becomes more systematic. Interestingly, the top-down-modulation does not interact with either social attention nor joint attention measueres. So, the effects by social influence is even when observers investigate the scene systematic relevant and cant be easily overwritten. 
