...
df_memory <-
    ...
    select(subject_id, trial_id, stim_id, stim_orientation,
           iti, condition_id, stim_cued, stim_object, stim_recall) %>%
  mutate(
...
# # ### equivalent to df.w.m =
# # df_memory %>%
# #   group_by(subject_id, stim_cued, condition_id) %>%
# #   summarise(stim_recall = sum(as.numeric(stim_recall))) %>%
# #   spread(stim_cued, stim_recall)
# 
# ##############################################################################
# 
# # ALL MEMORY DATA
